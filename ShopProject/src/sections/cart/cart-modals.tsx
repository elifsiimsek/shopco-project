...
export const CartModals = ({
  ...
  const validatePayment = () => {
    ...
    if (fullYear > 2036)
      return alert("Expiry date cannot exceed 10 years (2036).");
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-[6000] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-[380px] rounded-[32px] p-10 text-center shadow-2xl">
            ...
            <h3 className="text-xl font-black uppercase italic mb-3 text-black">
              Remove Item?
            </h3>
            <div className="flex flex-col gap-3">
              <button
                onClick={handleConfirmDelete}
                className="w-full bg-black text-white py-4 rounded-full font-black uppercase text-[11px]"
              >
                Yes, Remove
              </button>
              <button
                onClick={() => setDeleteConfirm(null)}
                className="w-full bg-transparent text-black/30 py-4 rounded-full font-black uppercase text-[11px]"
              >
                Keep it
              </button>
            </div>
          ...
          <div className="bg-white w-full max-w-[500px] rounded-[45px] p-10 md:p-12 relative shadow-2xl">
            ...
            {(user?.addresses?.length ?? 0) > 0 && (
              <div className="relative mb-6">
                <button
                  ...
                  className="w-full bg-[#FBFBFB] border-2 border-black/5 p-5 rounded-2xl flex justify-between items-center cursor-pointer"
                >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <select
                  className="bg-[#F5F5F5] p-5 rounded-2xl border-none outline-none font-black text-[12px] uppercase cursor-pointer"
                  value={address.city}
                  onChange={(e) => {
              ...
              {!isSavedAddrUsed && (
                ...
                >
                  <div
                    className={`w-6 h-6 border-2 rounded-lg flex items-center justify-center transition-all ${saveAddressDetails ? "bg-black border-black" : "border-black/10"}`}
                  >
                    {saveAddressDetails && (
              ...
              <button
                onClick={() =>
                  address.city && address.full
                    ...
                    : alert("Missing logistics.")
                }
                className="w-full bg-black text-white py-6 rounded-full font-black uppercase italic text-xs mt-4"
              >
                Proceed to Payment
              </button>
            </div>
          ...
          <div className="bg-white w-full max-w-[500px] rounded-[45px] p-10 md:p-12 relative shadow-2xl">
                  ...
                  <div className="absolute top-full left-0 w-full bg-white border border-black/10 mt-2 rounded-2xl shadow-2xl z-[60] max-h-[180px] overflow-y-auto">
                    {user?.savedCards?.map((c: PaymentCard) => (
                      ...
                      >
                        ...
                        <p className="text-[10px] font-bold text-black/30 m-0 uppercase mt-1 italic">
                          {c.holder}
                        </p>
                      </div>
                    ))}
            ...
            <div className="space-y-4">
              <input
                ...
                maxLength={16}
                className="w-full bg-[#F5F5F5] p-5 rounded-2xl border-none outline-none font-black text-[12px]"
                value={card.number}
                onChange={(e) => {
                  setCard({
                    ...card,
              ...
              {!isSavedCardUsed && (
                ...
                >
                  <div
                    className={`w-6 h-6 border-2 rounded-lg flex items-center justify-center transition-all ${saveCardDetails ? "bg-black border-black" : "border-black/10"}`}
                  >
                    {saveCardDetails && (
                      <FiCheckCircle className="text-white text-[10px]" />
                    )}
              ...
              <button
                ...
                className="w-full bg-black text-white py-6 rounded-full font-black uppercase italic text-xs mt-4 shadow-xl transition-all hover:scale-[1.01]"
              >
                Establish Order
              </button>
            </div>
