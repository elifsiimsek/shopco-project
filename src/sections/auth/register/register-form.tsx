        ...
        <form onSubmit={onSubmit} className="space-y-6" noValidate>
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="relative">
              ...
              />
              {errors.birthDate && <span className="absolute -bottom-5 left-0 text-[9px] font-bold text-red-500 leading-none">{errors.birthDate}</span>}
            </div>
            ...
            <div className="relative">
              ...
              />
              {errors.email && <span className="absolute -bottom-5 left-0 text-[9px] font-bold text-red-500 leading-none">{errors.email}</span>}
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="relative">
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-0 top-1/2 -translate-y-1/2 text-black/20 hover:text-black bg-transparent border-none cursor-pointer">
            ...
            <div className="flex gap-1 h-1 w-full bg-black/5 rounded-full overflow-hidden">
              {[...Array(4)].map((_, i) => (
                <div key={i} className={`h-full flex-1 transition-all duration-500 ${i < strengthScore ? (strengthScore <= 2 ? "bg-red-500" : strengthScore === 3 ? "bg-yellow-500" : "bg-green-500") : "bg-black/5"}`} />
              ))}
            </div>
          ...
          <div className="relative">
            ...
            />
            {errors.confirmPassword && <span className="absolute -bottom-5 left-0 text-[9px] font-bold text-red-500">{errors.confirmPassword}</span>}
          </div>
          ...
          <div className="pt-8 flex flex-col gap-6">
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full py-5 bg-black text-white rounded-full font-[1000] uppercase italic tracking-widest text-[11px] shadow-2xl hover:scale-[1.02] border-none cursor-pointer flex items-center justify-center disabled:opacity-50"
            >
              {isLoading ? "PROCESSING..." : "Establish Identity"} <FiArrowRight className="ml-2" />
            </Button>
            <p className="text-[10px] font-black uppercase tracking-widest text-black/30 text-center">
              Already verified? <Link to="/login" className="text-black font-black ml-2 underline underline-offset-4">Log In</Link>
            </p>
          </div>
        </form>
      </div>
