...
export default function LoginForm({
  ...
  const firstError = errors.general || errors.email || errors.password;
      ...
      <div className="max-w-[400px] mx-auto w-full space-y-12 py-10">
        ...
        <form onSubmit={onSubmit} className="space-y-10" noValidate>
          ...
          <div className="relative group text-left">
            <label
              htmlFor="login-email"
              className={`text-[9px] font-black uppercase tracking-[0.2em] mb-1 block cursor-pointer transition-colors ${
                errors.email ? "text-red-500" : "text-black/30"
              }`}
            >
              Identifier
            </label>
            <div className="relative">
              <input
                ...
                className={`w-full py-4 bg-transparent border-b-2 outline-none font-bold text-base transition-all text-black ${
                  errors.email ? "border-red-500" : "border-black/5 focus:border-black"
                }`}
                value={email}
              ...
              <FiMail
                ...
                className={`absolute right-0 top-1/2 -translate-y-1/2 transition-colors ${
                  errors.email ? "text-red-500" : "text-black/10"
                }`}
              />
            </div>
          </div>
          ...
          <div className="relative group text-left">
            <label
              ...
              className={`text-[9px] font-black uppercase tracking-[0.2em] mb-1 block cursor-pointer transition-colors ${
                errors.password ? "text-red-500" : "text-black/30"
              }`}
            >
              Security Key
            </label>
            <div className="relative">
              <input
                ...
                placeholder="••••••••"
                className={`w-full py-4 bg-transparent border-b-2 outline-none font-bold text-base transition-all text-black ${
                  errors.password ? "border-red-500" : "border-black/5 focus:border-black"
                }`}
                value={password}
