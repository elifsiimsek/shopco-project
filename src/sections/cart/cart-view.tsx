...
export default function CartView() {
  const { cart, removeFromCart, increase, decrease, completeOrder } = useCart();
  ...
  const handleFinalOrder = () => {
    ...
    const calculatedTotal = finalTotal + (finalTotal > 300 ? 0 : 15);
  ...
  if (cart.length === 0 && !orderSuccess)
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-4 text-center">
        ...
        <Link
          to="/shop"
          className="bg-black text-white px-10 py-4 rounded-full font-black uppercase italic text-[11px] no-underline shadow-xl"
        >
          Explore Archive
        </Link>
      </div>
    ...
    <div className="min-h-screen bg-white font-satoshi text-black py-10 md:py-16 text-left">
      <main className="max-w-[1240px] mx-auto px-4 md:px-6">
        <div className="flex items-center gap-4 mb-12">
          ...
          <h1 className="text-[36px] md:text-[52px] font-[900] uppercase m-0 tracking-tighter">
            Your Cart
          </h1>
        </div>
          ...
          <div className="max-w-[1000px] w-full grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="text-left space-y-10">
              ...
              <div className="flex flex-col gap-4 pt-4">
                ...
                <button
                  onClick={() => navigate("/")}
                  className="w-full max-w-[340px] text-[10px] font-black uppercase tracking-widest text-black/30 hover:text-black border-black/5 border py-4 rounded-full bg-transparent"
                >
                  Return to Terminal
                </button>
              </div>
