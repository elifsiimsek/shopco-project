...
const DEFAULT_REVIEWS: Review[] = [
  ...
  },
  {
    ...
    rating: 4,
    comment:
      "This t-shirt is a must-have for one who appreciates good design. The minimalistic yet stylish pattern caught my eye, and the fit is perfect. I can see the designer's touch in every aspect of this shirt.",
  },
  {
    id: "4",
    name: "Olivia P.",
...
export default function ProductDetailsView() {
    ...
    const loadData = async () => {
      if (product) {
        ...
        if (saved) {
          ...
          setProductReviews(product.reviews || DEFAULT_REVIEWS);
        }
      }
    };
    loadData();
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center font-black text-3xl opacity-10 uppercase tracking-widest bg-white text-black">
        Syncing Vault...
      </div>
    );
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 md:px-16 py-8 bg-white min-h-screen font-satoshi text-left text-black">
        ...
        <div className="flex border-b border-black/[0.08] mb-12 overflow-x-auto no-scrollbar">
          {["Product Details", "Rating & Reviews", "FAQs"].map((tab) => (
            <button
              ...
              className={`flex-1 pb-6 text-[16px] tracking-widest border-b-2 transition-all whitespace-nowrap px-8 bg-transparent cursor-pointer ${
                activeTab === tab
                  ? "text-black border-black font-black"
                  : "text-black/20 border-transparent font-bold"
              }`}
            >
              {tab}
            </button>
            ...
            <div className="animate-in fade-in duration-500">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-10">
                ...
                <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                  <button className="p-3 bg-shopGray-light rounded-full hover:bg-black/5 transition border-none cursor-pointer">
                    <SlidersHorizontal size={20} className="text-black" />
                  </button>
                  <div className="relative" ref={dropdownRef}>
                    <button
                      onClick={() => setIsSortOpen(!isSortOpen)}
                      className="flex items-center gap-4 bg-shopGray-light px-6 py-3 rounded-full font-medium cursor-pointer border-none text-black"
                    >
                      {sortOption}{" "}
                      <ChevronDown
                        size={16}
                      ...
                      <div className="absolute top-full right-0 mt-2 w-40 bg-white border border-black/5 rounded-2xl shadow-2xl z-50 overflow-hidden">
                        {["Latest", "Oldest"].map((opt) => (
                          <button
                            ...
                            className="w-full text-left px-6 py-3 hover:bg-shopGray-light font-bold text-sm border-none bg-white cursor-pointer transition-colors text-black"
                          >
                            {opt}
                          </button>
                        ))}
                  ...
                  <button
                    ...
                    className="bg-black text-white px-8 py-3 rounded-full font-medium hover:scale-[1.02] active:scale-95 transition border-none cursor-pointer"
                  >
                    Write a Review
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
                {sortedReviews.slice(0, visibleCount).map((review) => (
                  <div
                    key={review.id}
                    className="p-8 border border-black/[0.08] rounded-[32px] bg-white relative group hover:shadow-xl transition-all h-full flex flex-col justify-between text-left"
                  >
                    <div>
                      <div className="absolute top-8 right-8 flex items-center gap-2">
                        {user?.email === review.email && (
                          <button
                            ...
                            className="p-2 text-red-500 bg-red-50 rounded-full hover:bg-red-500 hover:text-white transition border-none cursor-pointer"
                          >
                            <Trash2 size={16} />
                          </button>
                        )}
                    ...
                    <p className="mt-8 text-[11px] font-black text-black/20 tracking-widest uppercase">
              ...
              {visibleCount < sortedReviews.length && (
                <div className="flex justify-center mt-12">
                  <button
                    ...
                    className="px-14 py-4 border border-black/10 rounded-full bg-white font-medium hover:bg-black hover:text-white transition cursor-pointer text-black"
                  >
                    Load More Reviews
                  </button>
                </div>
      ...
      {relatedProducts.length > 0 && (
        <div className="mt-32">
          <h2 className="text-[40px] md:text-[48px] font-black text-center mb-16 uppercase">
            You Might Also Like
          </h2>
